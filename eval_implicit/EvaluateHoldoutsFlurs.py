        ...
        for b, bucket in enumerate(self.buckets):
            ...
            if b >= self.cold_start_buckets:
                joblib.dump(self.model.data, 
                            self.filepath+'model_data_b'+str(b-self.cold_start_buckets)+'.joblib')  
                
                self._evaluate_save(n_bucket=b)


    def _evaluate_save(self, n_bucket):
        # after learning a bucket, evaluate model over every holdout     
        evaluate_time = [] 
        time_get_tuple = []
        time_recommend = []
        ...
        for i, hd in enumerate(self.holdouts):
            ...
            joblib.dump(rec_lists,
                        self.filepath+'rec_lists_b'+str(n_bucket-self.cold_start_buckets)+'_h'+str(i)+'.joblib') 
