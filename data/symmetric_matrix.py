...
class SymmetricMatrix:
    ...
    def __init__(self, num_entities: int = 0):
        self.size = (num_entities + 1) * 2 # (number of entities + 1) *2
        self.matrix = np.zeros((self.size, self.size))
        self.max_id = num_entities - 1 # max id: number of entities -1
    ...
    def GetRow(self, i: int):
        return self.matrix[i][:self.max_id + 1] # returns the row for an entity, limited to max_id
